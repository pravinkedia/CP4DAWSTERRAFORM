...
locals {
  ...
  private_subnets     = var.az == "multi_zone" ? [local.private_subnet1_id, local.private_subnet2_id, local.private_subnet3_id] : [local.private_subnet1_id]
...
resource "local_sensitive_file" "aws_creds_file" {
...
module "network" {
  ...
  depends_on = [
    local_sensitive_file.aws_creds_file,
    local_file.aws_config_file,
...
module "fsx" {
  ...
  fsx_endpoint_cidr        = var.fsx_endpoint_cidr
  fsx_admin_password       = var.fsx_admin_password
  trident_operator_version = var.trident_operator_version
  cluster_name             = var.cluster_name
  login_cmd                = local.login_cmd
  vpc_id                   = local.vpc_id
  region                   = var.region
...
module "cpd" {
  ...
  watson_knowledge_catalog_core = var.watson_knowledge_catalog_core
